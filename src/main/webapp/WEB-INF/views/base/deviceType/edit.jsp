...
<body>
<form action="/cyx/deviceType/update" method="post" id="form" >
<input type="hidden" name="id" value="${deviceType.id}">
<input type="hidden" name="ifdelete" value="${deviceType.ifdelete}">
<input type="hidden" name="createtime" value="${deviceType.createtime}">
<table>
	<tbody>
		<tr>
			<th>名称</th>
			<td>
				<input type="text" name="name" value="${deviceType.name}"/>
			</td>
			<th>排序</th>
			<td>
				<input type="text" name="sort" value="${deviceType.sort}"/>
			</td>
		</tr>
		<tr>
			<th>创建人</th>
			<td>
				<input type="text" value="${deviceType.createUser.name}" readonly="readonly">
				<input type="hidden" name="createUser.id" value="${deviceType.createUser.id}">
			</td>
			<td colspan="2"></td>
		</tr>
